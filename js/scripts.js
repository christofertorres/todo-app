...
const saveTodo = (text, done = 0, save = 1, id = String(Date.now())) => {
  ...
  if (save) {
    saveTodoLocalStorage({ id, text, done: 0 });
  }
...
const updateTodo = (text) => {
  const todo = document.querySelector(`.todo[data-id="${currentEditId}"]`);

  if (todo) {
    todo.querySelector("h3").innerText = text;
    updateTodoLocalStorage(currentEditId, text);
  }
};
